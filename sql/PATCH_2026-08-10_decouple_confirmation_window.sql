-- ============================================================================
-- PATCH — decouples Today's Confirmation from the shared time window
-- entirely. Confirmation is now gated ONLY by "already submitted"
-- (confirmation_locked) and "No Food enabled for this specific meal" —
-- never by booking_open_time/booking_close_time. Tomorrow's Booking still
-- uses that window exactly as before; nothing about it changes here.
--
-- Symptom this fixes: submitting Today's Confirmation (or the window
-- simply closing from normal time passing) could make Tomorrow's Booking
-- look locked too, even though nothing about the confirmation actually
-- caused it — they were both checking the same window, so one part of
-- the day ending affected both features at once.
--
