-- ============================================================================
-- PATCH — Today's Confirmation is now pre-filled from yesterday's booking
-- and locked unless No Food is enabled for that specific meal. Run this
-- whole block in the SQL Editor. Safe to re-run.
--
-- After running this, redeploy the updated lock-confirmations function:
--   supabase functions deploy lock-confirmations
-- (it now auto-copies booking_status into confirmed_status overnight for
-- any meal where No Food is disabled, so the fine calculation still sees a
-- real confirmed value instead of "never confirmed").
-- ============================================================================
...
begin
  ...
  -- NEW: a student may only write confirmed_status at all when No Food is
  -- enabled for that specific meal. Otherwise the meal is frozen to
  -- whatever was booked yesterday — see lock-confirmations for the
  -- overnight auto-copy that keeps the fine calculation correct.
