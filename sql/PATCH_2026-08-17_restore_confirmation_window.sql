-- ============================================================================
-- PATCH -- restores the shared meal-selection window check for Today's
-- Confirmation (in addition to the existing lock + No Food checks, which
-- are unchanged). Confirmation now uses the SAME window as Tomorrow's
-- Booking (booking_open_time/booking_close_time) again -- this was
-- deliberately removed in an earlier patch, then explicitly re-requested.
-- No other rule in this trigger changes.
