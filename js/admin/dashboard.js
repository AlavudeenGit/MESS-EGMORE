...
// ============================================================================
import { supabase, MEAL_TYPES, MEAL_LABELS, EXPENSE_CATEGORY_LABELS } from '../config.js';
import { requireRole, initTheme, toggleTheme, todayISO, currency, formatDate } from '../utils.js';
...
document.getElementById('settingsShortcut').addEventListener('click', () => navigate('settings'));
...
async function renderHome(root) {
  ...
  const [
    ...
    supabase.from('bookings').select('meal_type, booking_status').eq('date', today),
    supabase.from('bookings').select('meal_type, confirmed_status').eq('date', today),
    supabase.from('fines').select('amount').gte('date', monthStart),
    supabase.from('expenses').select('amount').gte('date', monthStart),
    supabase.from('payments').select('paid_amount').eq('month_year', monthStart),
    supabase.from('students').select('name, created_at').order('created_at', { ascending: false }).limit(5),
    supabase.from('expenses').select('category, amount, date').order('created_at', { ascending: false }).limit(5),
  ]);
  ...
  const totalRevenueAmt = (monthPayments || []).reduce((s, p) => s + Number(p.paid_amount || 0), 0) + totalFineAmt;
...
function countByMeal(rows, field) {
  ...
  (rows || []).forEach(r => { if (r[field] && r[field] !== 'no') out[r.meal_type]++; });
  return out;
}
...
async function drawBookingTrend() {
  ...
  const { data } = await supabase.from('bookings').select('date, booking_status').in('date', days);
  const counts = days.map(d => (data || []).filter(r => r.date === d && r.booking_status && r.booking_status !== 'no').length);
  const ctxEl = document.getElementById('chartBookings');
  if (!ctxEl || !window.Chart) return;
  new Chart(ctxEl, {
    type: 'line',
    data: { labels: days.map(d => formatDate(d, { month: 'short', day: '2-digit', year: undefined })), datasets: [{ label: 'Bookings', data: counts, borderColor: '#2F5233', backgroundColor: 'rgba(47,82,51,0.12)', fill: true, tension: 0.35 }] },
...
function drawExpensePie(monthExpenses) {
  ...
  new Chart(ctxEl, {
    type: 'doughnut',
    data: { labels, datasets: [{ data: Object.values(byCat), backgroundColor: ['#2F5233', '#E4A72E', '#C0392B', '#7A4FAE', '#8A8578', '#6FA97A', '#B392DE', '#3B342C', '#E5786A'] }] },
