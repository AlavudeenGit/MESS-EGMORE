...
export async function renderStudents(root) {
  ...
  async function load() {
    let query = supabase.from('students').select('*').neq('status', 'pending').order('created_at', { ascending: false });
    if (state.status) query = query.eq('status', state.status);
    const { data, error } = await query;
    if (error) { document.getElementById('studentsTable').innerHTML = `<p class="text-danger">Failed to load students.</p>`; return; }

    const filtered = state.search
      ? data.filter(s => s.name.toLowerCase().includes(state.search) || s.room_number.toLowerCase().includes(state.search))
      : data;
    ...
    const columns = [
      ...
      { key: 'status', label: 'Status', render: r => `<span class="badge ${r.status === 'active' ? 'badge-yes' : 'badge-no'}">${r.status}</span>` },
    ...
    document.getElementById('studentsTable').innerHTML = renderTable(columns, filtered, { emptyMessage: 'No students found' });
    wireRowActions(filtered);
  }

  function wireRowActions(rows) {
    document.querySelectorAll('[data-act="edit"]').forEach(btn => btn.onclick = () => openEditModal(rows.find(r => r.id === btn.dataset.id), load));
    document.querySelectorAll('[data-act="toggle"]').forEach(btn => btn.onclick = () => toggleStatus(rows.find(r => r.id === btn.dataset.id), load));
    document.querySelectorAll('[data-act="delete"]').forEach(btn => btn.onclick = () => deleteStudent(btn.dataset.id, load));
  }
  ...
  document.getElementById('addStudentBtn').addEventListener('click', () => openAddModal(load));
...
function openAddModal(onSaved) {
  ...
  body.querySelector('#createStudentBtn').onclick = async () => {
    ...
    if (!payload.name || !payload.room_number || !payload.mobile || !payload.email || !payload.password) {
      toast.error('All fields are required'); return;
    }
    btn.disabled = true; btn.textContent = 'Creating…';
    const { data, error } = await supabase.functions.invoke('admin-create-student', { body: payload });
...
async function toggleStatus(student, onDone) {
  ...
  const ok = await confirmDialog(`Mark ${student.name} as ${newStatus}?`, { danger: newStatus === 'inactive', confirmLabel: 'Yes, continue' });
  if (!ok) return;
  const { error } = await supabase.from('students').update({
    status: newStatus,
    deactivated_at: newStatus === 'inactive' ? new Date().toISOString().slice(0, 10) : null
  }).eq('id', student.id);
...
async function deleteStudent(id, onDone) {
  const ok = await confirmDialog('This will permanently remove the student record. This cannot be undone from the UI. Continue?', { confirmLabel: 'Delete' });
  if (!ok) return;
