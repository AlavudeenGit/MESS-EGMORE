...
export async function renderRegistrations(root) {
  ...
  async function load() {
    const { data, error } = await supabase.from('students').select('*').eq('status', 'pending').order('created_at', { ascending: false });
    if (error) { document.getElementById('regTable').innerHTML = `<p class="text-danger">Failed to load registrations.</p>`; return; }
    ...
    const columns = [
      ...
      { key: 'created_at', label: 'Requested', render: r => formatDate((r.created_at || '').slice(0, 10)) },
    ...
    document.getElementById('regTable').innerHTML = renderTable(columns, data, { emptyMessage: 'No pending registrations 🎉' });

    document.querySelectorAll('[data-act="approve"]').forEach(b => b.onclick = () => approve(b.dataset.id, load));
    document.querySelectorAll('[data-act="reject"]').forEach(b => b.onclick = () => reject(b.dataset.id, load));
    document.querySelectorAll('[data-act="edit"]').forEach(b => b.onclick = () => openEdit(data.find(r => r.id === b.dataset.id), load));
  }
...
async function approve(id, onDone) {
  const ok = await confirmDialog('Approve this registration? The student will be able to log in immediately.', { danger: false, confirmLabel: 'Approve' });
  if (!ok) return;
...
async function reject(id, onDone) {
  const ok = await confirmDialog('Reject this registration? The student will not be able to log in.', { confirmLabel: 'Reject' });
  if (!ok) return;
