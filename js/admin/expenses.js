...
export async function renderExpenses(root) {
  ...
  async function load() {
    ...
    if (error) {
      document.getElementById("expensesTable").innerHTML =
        `<p class="text-danger">Failed to load expenses.</p>`;
      return;
    }
    ...
    document
      ...
      .forEach((b) => (b.onclick = () => deleteExpense(b.dataset.id, load)));
  }
  ...
  document
    ...
    .addEventListener("click", () => openExpenseModal(null, load));
...
async function deleteExpense(id, onDone) {
  const ok = await confirmDialog("Delete this expense record?", {
    confirmLabel: "Delete",
  });
  if (!ok) return;
