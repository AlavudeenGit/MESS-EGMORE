...
export async function renderPayments(root) {
  ...
  const state = { search: '', month: defaultMonth, active: '', paid: '', ate: '' };
  ...
  async function load() {
    ...
    const monthEnd = new Date(Number(state.month.split('-')[0]), Number(state.month.split('-')[1]), 0).toISOString().slice(0, 10);

    let studentQuery = supabase.from('students').select('*').neq('status', 'pending');
    if (state.active) studentQuery = studentQuery.eq('status', state.active);
    const { data: students, error: sErr } = await studentQuery;
    if (sErr) { document.getElementById('paymentsTable').innerHTML = `<p class="text-danger">Failed to load students.</p>`; return; }

    const { data: payments } = await supabase.from('payments').select('*').eq('month_year', monthStart);
    const { data: eaten } = await supabase.from('bookings').select('student_id')
    ...
    const { data: doubles } = await supabase.from('bookings').select('student_id, meal_type')
      .gte('date', monthStart).lte('date', monthEnd).eq('confirmed_status', 'double');
    const doubleCounts = {}; // student_id -> { breakfast: n, lunch: n, dinner: n }
    (doubles || []).forEach(r => {
      doubleCounts[r.student_id] = doubleCounts[r.student_id] || { breakfast: 0, lunch: 0, dinner: 0 };
      doubleCounts[r.student_id][r.meal_type]++;
    });

    let rows = students.map(s => {
      const pay = (payments || []).find(p => p.student_id === s.id);
      const dCounts = doubleCounts[s.id] || { breakfast: 0, lunch: 0, dinner: 0 };
      const estCost = dCounts.breakfast * rates.breakfast + dCounts.lunch * rates.lunch + dCounts.dinner * rates.dinner;
      return {
        student: s,
    ...
    if (state.search) rows = rows.filter(r => r.student.name.toLowerCase().includes(state.search) || r.student.room_number.toLowerCase().includes(state.search));
  ...
  function renderRows(rows, monthStart) {
    const columns = [
      ...
      { key: 'ate', label: 'Ate?', render: r => r.ate ? '<span class="badge badge-yes">Yes</span>' : '<span class="badge badge-no">No</span>' },
      { key: 'double', label: 'Double Meals', render: r => r.doubleTotal === 0 ? '—' :
      ...
      { key: 'estCost', label: 'Est. Double Cost', render: r => r.estCost > 0
        ? `<span class="badge badge-locked" title="Reference only — not added automatically">${currency(r.estCost)}</span>`
        : '—' },
      { key: 'amount', label: 'Mess Amount', render: r => `<input type="number" class="mono" style="width:100px;min-height:36px;border:1px solid var(--color-border);border-radius:8px;padding:0 8px;" data-field="amount" data-id="${r.student.id}" value="${r.payment.mess_amount || ''}">` },
      { key: 'paid', label: 'Paid Amount', render: r => `<input type="number" class="mono" style="width:100px;min-height:36px;border:1px solid var(--color-border);border-radius:8px;padding:0 8px;" data-field="paid" data-id="${r.student.id}" value="${r.payment.paid_amount || ''}">` },
      { key: 'status', label: 'Status', render: r => `<span class="badge ${r.payment.status === 'paid' ? 'badge-yes' : r.payment.status === 'partial' ? 'badge-pending' : 'badge-no'}">${r.payment.status}</span>` },
      { key: 'save', label: '', render: r => `<button class="btn btn-primary btn-sm" data-act="save" data-id="${r.student.id}">Save</button>` },
    ];
    ...
    document.querySelectorAll('[data-act="save"]').forEach(btn => {
      btn.onclick = async () => {
        ...
        const amount = Number(document.querySelector(`[data-field="amount"][data-id="${id}"]`).value) || 0;
        const paid = Number(document.querySelector(`[data-field="paid"][data-id="${id}"]`).value) || 0;
        const status = paid <= 0 ? 'unpaid' : paid >= amount && amount > 0 ? 'paid' : 'partial';

        const { error } = await supabase.from('payments').upsert({
          student_id: id, month_year: monthStart, mess_amount: amount, paid_amount: paid,
          status, payment_date: paid > 0 ? todayISO() : null
        }, { onConflict: 'student_id,month_year' });

        if (error) { toast.error('Could not save payment'); return; }
  ...
  document.getElementById('searchName').addEventListener('input', debounce(e => { state.search = e.target.value.toLowerCase(); load(); }, 250));
  document.getElementById('filterMonth').addEventListener('change', e => { state.month = e.target.value; load(); });
  document.getElementById('filterActive').addEventListener('change', e => { state.active = e.target.value; load(); });
  document.getElementById('filterPaid').addEventListener('change', e => { state.paid = e.target.value; load(); });
  document.getElementById('filterAte').addEventListener('change', e => { state.ate = e.target.value; load(); });
