...
export async function login(email, password) {
  ...
  // check admin first
  const { data: admin } = await supabase.from('admins').select('id').eq('id', uid).maybeSingle();
  if (admin) {
  ...
  // then student
  const { data: student } = await supabase.from('students').select('status').eq('id', uid).maybeSingle();
  if (student) {
 ...
 */
export async function submitRegistration({ name, room_number, mobile, email, password }) {
...
export async function redirectIfLoggedIn() {
  ...
  const { data: admin } = await supabase.from('admins').select('id').eq('id', uid).maybeSingle();
  if (admin) { window.location.href = 'admin-dashboard.html'; return; }
  const { data: student } = await supabase.from('students').select('status').eq('id', uid).maybeSingle();
  if (student && student.status === 'active') { window.location.href = 'student-dashboard.html'; }
}
