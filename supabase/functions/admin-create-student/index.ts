...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: adminRow } = await adminClient.from('admins').select('id').eq('id', user.id).maybeSingle();
    if (!adminRow) return json({ ok: false, error: 'Caller is not an admin' }, 403);
    ...
    const { error: insertError } = await adminClient.from('students').insert({
      id: created.user.id, name, room_number, mobile, email, status: 'active'
    });
    if (insertError) {
