...
// Scheduled once daily, shortly after `settings.confirmation_deadline`.
...
Deno.serve(async (req) => {
  try {
    ...
    const { data: existingRows, error: existingError } = await supabase
      .from('bookings').select('id, student_id, meal_type, booking_status').eq('date', today);
    if (existingError) throw existingError;

    const existingKey = new Set((existingRows || []).map(r => `${r.student_id}:${r.meal_type}`));
    ...
    for (const s of students || []) {
      for (const meal of MEAL_TYPES) {
        if (!existingKey.has(`${s.id}:${meal}`)) {
          missing.push({
            student_id: s.id, date: today, meal_type: meal,
    ...
    if (missing.length) {
      const { error } = await supabase.from('bookings')
        .upsert(missing, { onConflict: 'student_id,date,meal_type', ignoreDuplicates: true });
      if (error) throw error;
    }
    ...
    for (const s of students || []) {
      const { error } = await supabase.rpc('recompute_daily_fine', {
        p_student_id: s.id, p_date: today
      });
      if (error) { console.error('recompute_daily_fine failed for', s.id, error); continue; }
      fined++;
    }

    return new Response(JSON.stringify({
      ok: true, date: today, locked: existingIds.length, defaulted: missing.length, recomputed: fined
