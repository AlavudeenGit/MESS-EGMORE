...
Deno.serve(async (req) => {
  try {
    ...
    const { data: existingRows, error: existingError } = await supabase
      .from('bookings').select('id, student_id, meal_type').eq('date', tomorrow);
    ...
    for (const s of students || []) {
      for (const meal of MEAL_TYPES) {
        if (!existingKey.has(`${s.id}:${meal}`)) {
          missing.push({
            student_id: s.id, date: tomorrow, meal_type: meal,
    ...
    return new Response(JSON.stringify({
      ok: true, date: tomorrow, locked: existingIds.length, defaulted: missing.length
